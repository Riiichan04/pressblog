...
import {
    getAllAdminComments, approveComment, safeDeleteComment, restoreComment
...
export default function CommentList({ filterStatus, postSlug, onClearPostFilter }: CommentListProps) {
    ...
    const executeDelete = async () => {
        ...
        try {
            ...
            if (filterStatus === "PENDING") {
                ...
                setComments(prev => prev.map(c => c.id === commentToDelete ? { ...c, deleted: true } : c));
            }
        } catch {
            toast.error(t("comments.messages.error"));
        } finally {
    ...
    const getStatusBadge = (status: string, isDeleted: boolean) => {
        if (isDeleted) return <Badge variant="destructive">Đã xóa</Badge>;
        switch (status) {
            case "APPROVED": return <Badge className="bg-green-500 hover:bg-green-600">Đã duyệt</Badge>;
            case "PENDING": return <Badge variant="secondary" className="text-orange-500 hover:text-orange-600">Chờ duyệt</Badge>;
    ...
    return (
        <div className="border rounded-lg bg-card overflow-hidden p-4">
            {postSlug && (
                <div className="mb-4 flex items-center justify-between rounded-md bg-blue-50/50 p-3 border border-blue-100 dark:bg-blue-950/20 dark:border-blue-900/50">
                    <div className="flex items-center text-sm text-blue-800 dark:text-blue-300">
                        ...
                        <span>Đang lọc bình luận theo bài viết: <span className="font-semibold">{postSlug}</span></span>
                    </div>
                    <Button variant="ghost" size="sm" onClick={onClearPostFilter} className="h-8 text-blue-800 hover:bg-blue-100 dark:text-blue-300 dark:hover:bg-blue-900/50">
                        <X className="mr-1 h-4 w-4" /> Bỏ lọc
                    </Button>
                </div>
            )}
                            ...
                            <TableRow key={cmt.id} className={cmt.isDeleted ? "opacity-60 bg-muted/30" : ""}>
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        {cmt.parentId && (
                                            <div className="flex items-center gap-1 w-fit text-[11px] font-medium bg-muted/70 text-muted-foreground px-2 py-1 rounded-md border border-muted-foreground/20">
                                                <span className="opacity-70">Trả lời:</span>
                                ...
                                <TableCell className="text-right">
                                    <div className="flex justify-end gap-2">
                                        <Button variant="outline" size="icon" onClick={() => setSelectedComment(cmt)} title="Xem chi tiết">
                                            <Eye className="h-4 w-40" />
                                        </Button>
            ...
            <AlertDialog open={commentToDelete !== null} onOpenChange={(open) => !open && setCommentToDelete(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>{t("comments.actions.deleteConfirmTitle")}</AlertDialogTitle>
                        <AlertDialogDescription>{t("comments.actions.deleteConfirm")}</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>{t("comments.actions.cancel")}</AlertDialogCancel>
                        <AlertDialogAction onClick={executeDelete} className="bg-red-600 hover:bg-red-700 text-white">{t("comments.actions.delete")}</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
            ...
            <Dialog open={selectedComment !== null} onOpenChange={(open) => !open && setSelectedComment(null)}>
                <DialogContent className="sm:max-w-150">
                    ...
                    {selectedComment && (
                        <div className="space-y-4 py-4">
                            <div className="grid grid-cols-2 gap-4 text-sm bg-muted/30 p-3 rounded-lg border">
                                <div><span className="text-muted-foreground font-medium">Người đăng:</span> <span className="font-semibold">{selectedComment.authorName}</span></div>
                                <div><span className="text-muted-foreground font-medium">Trạng thái:</span> <span className="ml-2">{getStatusBadge(selectedComment.status, selectedComment.isDeleted)}</span></div>
                                <div className="col-span-2"><span className="text-muted-foreground font-medium">Bài viết:</span> {selectedComment.postName || selectedComment.postName}</div>
                                <div className="col-span-2"><span className="text-muted-foreground font-medium">Ngày đăng:</span> {new Date(selectedComment.createdAt).toLocaleString("vi-VN")}</div>
                            </div>

                            {selectedComment.parentId && (
                                <div className="bg-muted/60 p-3 rounded-md text-sm border-l-4 border-blue-400">
                                    <div className="font-semibold text-muted-foreground text-xs mb-1 uppercase">Đang trả lời bình luận:</div>
                                    <div className="italic text-muted-foreground">&quot;{selectedComment.parentContent}&quot;</div>
                                </div>
                            )}

                            <div>
                                <div className="font-semibold text-sm mb-2 text-foreground">Nội dung:</div>
                    ...
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setSelectedComment(null)}>{t("comments.actions.cancel")}</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </div>
